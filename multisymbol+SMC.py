...
PAIR_CONFIG = {
    ...
    "GBP/USD": {"atr_mult": 2.3, "min_score": 60},
...
def multi_timeframe_confirm(symbol):
    ...
    def assess(df):
        ...
        if wick_range == 0 or real_body / wick_range < 0.3 or last['ATR'] > last['close'] * 0.04:
            return 0
        ...
        total = ma_score + rsi_score + candle + breakout + macd_score + bb_score + stoch_score
        return total

    total_score = assess(df_5) + assess(df_15) + assess(df_1h)
    normalized_score = int((total_score + 21) / 42 * 100)  # Confidence out of 100
    return normalized_score, df_15

# === SIGNAL ENGINE ===
def signal_engine(symbol):
    now = datetime.datetime.now()
    expires = now + datetime.timedelta(minutes=15)
    confidence_score, df = multi_timeframe_confirm(symbol)
    last = df.iloc[-1]
    ...
    if confidence_score >= min_score and entry > recent_high:
        sl = round(entry - atr * atr_mult, 4)
        ...
        return {
            ...
            "timestamp": now, "expires": expires, "rrr": rrr
        }
    elif confidence_score <= -min_score and entry < recent_low:
        sl = round(entry + atr * atr_mult, 4)
    ...
    elif confidence_score >= min_score - 10:
        return {"type": "Weak Buy", "entry": entry, "tp1": None, "tp2": None, "tp3": None, "sl": None, "timestamp": now, "expires": expires}
    elif confidence_score <= -(min_score - 10):
        return {"type": "Weak Sell", "entry": entry, "tp1": None, "tp2": None, "tp3": None, "sl": None, "timestamp": now, "expires": expires}
    else:
        return {"type": "No Clear Signal", "entry": entry, "tp1": None, "tp2": None, "tp3": None, "sl": None, "timestamp": now, "expires": expires}
...
def update_analysis():
    ...
    if last_refresh_time and (now - last_refresh_time).total_seconds() < 60:
        signal_label.config(text="Please wait 60s between refreshes.")
        return
    ...
    try:
        ...
        tp1, tp2, tp3, sl = signal_data["tp1"], signal_data["tp2"], signal_data["tp3"], signal_data["sl"]
...
# === GUI SETUP ===
root = tk.Tk()
root.title("STEEL TECH MultiSignal Bot")
...
ttk.Label(root, text="MultiSignal Bot", font=("Segoe UI", 22, "bold")).pack(pady=10)
